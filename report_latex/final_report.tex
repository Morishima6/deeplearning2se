...
\figuretodo{LOSVER-Light 整体流程图}{建议制作一张流程图：CodeXGLUE/Devign 原始函数 $\rightarrow$ 行级风险评分 $\rightarrow$ top-k 风险行 $\rightarrow$ Vanilla/Tag/Tag+Prefix 三种输入 $\rightarrow$ Qwen2.5-Coder + QLoRA 分类器 $\rightarrow$ 指标与错例分析。}{fig:method-flow}
...
\figuretodo{主实验指标对比图}{建议制作分组柱状图：横轴为 Metrics-Baseline、Vanilla Qwen、LOSVER-Light Tag、LOSVER-Light Tag+Prefix，纵轴展示 F1、ROC-AUC、PR-AUC 三个指标。}{fig:main-results}
...
\figuretodo{top-k 消融趋势图}{建议制作折线图或柱状图：横轴为 top-k=3/5/8，纵轴展示 F1、Precision、Recall，突出 top-k=5 的折中效果。}{fig:topk-ablation}
...
\figuretodo{人工错例类别分布图}{建议根据 \texttt{reports/tables/manual\_error\_summary.csv} 制作水平柱状图，展示长函数、上下文依赖、错误处理误报、弱行级信号等类别的数量。}{fig:error-analysis}
